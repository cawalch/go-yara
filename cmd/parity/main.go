        ...
        for _, r := range splitCSV(*rulesCSV) {
            ...
            if *skipIncludes {
                if has, err := fileHasInclude(r); err == nil {
                    if has { skipped.includes++; continue }
                } else {
            ...
            if *skipModules {
                if has, err := fileHasImport(r); err == nil {
                    if has { skipped.modules++; continue }
                } else {
...
func parseYaraOutput(stdout string) []string {
    ...
    for k := range rules { out = append(out, k) }
    sort.Strings(out)
    return out
}
...
func parseGoYaraMatches(stdout string) []string {
    ...
    for k := range matches { out = append(out, k) }
    sort.Strings(out)
    return out
}
