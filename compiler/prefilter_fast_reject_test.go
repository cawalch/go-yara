...
`)
	...
	inputs := map[string][]byte{
		"reject":    []byte(`{"level":"info","message":"request completed","status":200}`),
		"gate_pass": []byte(`{"level":"debug","field":"api_key","status":"redacted"}`),
		"match":     []byte(`{"level":"warn","field":"api_key","value":"secret_value"}`),
	}
