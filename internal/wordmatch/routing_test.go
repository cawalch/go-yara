...
//nolint:revive // argument-limit: parity helper
...
func TestRoutingDeepCorrelatedContexts(t *testing.T) {
	...
	if maxDepth < 8 || inspectRoutes(program).MaxLeaf > 8 {
		t.Fatalf("insufficient routing depth or oversized leaf: depth=%d, %+v", maxDepth, inspectRoutes(program))
	}
	scanner := program.NewScanner()
	...
	t.Logf("16384 positives and 16384 invalid checksums, each code at all eight alignments; depth=%d, %+v", maxDepth, inspectRoutes(program))
}
...
func inspectRoutes(p *RoutedProgram) (stats struct{ Nodes, Leaves, MaxLeaf int }) {
	stats.Nodes = len(p.nodes) - 1
	for _, node := range p.nodes[1:] {
		if node.count != 0 {
			stats.Leaves++
			stats.MaxLeaf = max(stats.MaxLeaf, int(node.count))
		}
	}
	return
}
...
//nolint:nestif // independent exhaustive oracle
...
func TestSharedPlanAndScannerReuse(t *testing.T) {
	anchor := []byte("Reusable_Anchor_0123456789")
	rules := []Rule{{All: []Pattern{{Any: []Sequence{{
		{Literal: []byte("<")}, {Set: routingSet('0', '9'), Min: 0, Max: math.MaxInt},
		{Literal: anchor, NoCase: true}, {Set: routingSet('0', '9'), Min: 1, Max: 3}, {Literal: []byte(">")},
	}}}}}}
	p, err := CompileRouted(rules)
	if err != nil {
		t.Fatal(err)
	}
	rules[0].All[0].Any[0] = nil
	clear(anchor)
	for i := 0; i < 8; i++ {
		t.Run(fmt.Sprint(i), func(t *testing.T) {
			...
			for _, item := range []struct {
				...
				{"<12reusable_ANCHOR_01234567893>", Match},
				{"<Reusable_Anchor_0123456789>", NoMatch},
				{"", NoMatch},
				{"<Reusable_Anchor_01234567891>", Match},
			} {
				if got := scanner.Match([]byte(item.data)); got != item.want {
					t.Fatalf("%q: got %v, want %v", item.data, got, item.want)
				}
			}
		})
	}
