...
mkdir -p results
{
  ...
  sha256sum compiler/{event_study,event_guard_study,two_stage_study}_test.go .github/scripts/two-stage-study.sh
} > results/environment.txt
[[ "$(go env GOOS)/$(go env GOARCH)" == linux/amd64 ]] || { echo 'Linux amd64 required'; exit 1; }

variants=(baseline full atoms)
revisions=(fcd4e78754aeba52bc90d0cb36a2bc3e28bb7bf2 "$FULL_REF" "$ATOMS_REF")
...
for i in "${!variants[@]}"; do
  ...
  cp compiler/{event_study,event_guard_study,two_stage_study}_test.go "$checkout/compiler/"
...
for round in 1 2 3 4 5 6; do
  order=(baseline full atoms)
  if ((round % 2 == 0)); then order=(atoms full baseline); fi
