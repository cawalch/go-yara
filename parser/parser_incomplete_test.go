...
const (
	...
	parseError parseExpectation = true
)

// assertParseResult verifies whether parsing succeeds or fails as expected.
//
...
func TestRuleWithoutCondition(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			expect:      parseOK,
			description: "Defers unresolved string references to semantic validation",
		},
		{
...
func TestPartialCondition(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			rule:        `rule test { condition: # }`,
			expect:      parseOK,
			description: "Bare string-count placeholders are parsed before loop-context validation",
		},
		{
