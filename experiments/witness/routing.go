...
type RoutingStats struct{ Nodes, Leaves, MaxLeaf, Work int }
...
func CompileRouted(rules []Rule) (*RoutedProgram, error) {
	...
	for _, lane := range base.lanes {
		if lane.width != 8 {
			return nil, fmt.Errorf("%w: short witness lane", ErrIneligible)
		}
	}
	p := &RoutedProgram{base: base, roots: make([]uint32, len(base.postings)), nodes: []routeNode{{}}}
	for _, lane := range base.lanes {
		for _, slot := range lane.table {
			var ids []uint32
	...
	p.stats.Nodes = len(p.nodes) - 1
	return p, nil
}

func (p *RoutedProgram) Stats() RoutingStats { return p.stats }
func (p *RoutedProgram) NewScanner() *RoutedScanner {
	return &RoutedScanner{plan: p, exact: p.base.NewScanner()}
}
...
func (s *RoutedScanner) Match(data []byte) Decision {
	...
	if len(p.base.lanes) == 0 {
		return NoMatch
	}
	lane := &p.base.lanes[0]
	mask := uint64(len(lane.table) - 1)
	for pos := 0; pos <= len(data)-8; pos += 8 {
		word := binary.LittleEndian.Uint64(data[pos:]) | 0x2020202020202020
